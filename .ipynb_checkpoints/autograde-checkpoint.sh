...
do
    ...
    for file in $TESTDIR/*; do
        ...
        then
            ...
            ANS=$CASE.ans
            rm $MY
            $EXEC -i $IN -o $MY -t $THREADS --alg=$alg
            if cmp --silent $MY $ANS;
            then 
                SCORE=$(($SCORE+5))
                echo -e "Algorithm: $alg, $CASE ..... ${GREEN}Pass${NOCOLOR}"
            else 
                echo -e "Algorithm: $alg, $CASE ..... ${RED}FAIL${NOCOLOR}"
            fi
        fi
    done

    echo -e "Algorithm: $alg, ${YELLOW}Score:" $SCORE "${NOCOLOR}"
